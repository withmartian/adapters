...
def test_sync(vcr, model_path: str):
    ...
    adapter_response = adapter.execute_sync(SIMPLE_CONVERSATION_MULTIPLE_SYSTEM)
...
async def test_async(vcr, model_path: str):
    ...
    adapter_response = await adapter.execute_async(SIMPLE_CONVERSATION_MULTIPLE_SYSTEM)
